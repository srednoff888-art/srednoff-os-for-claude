...
- **Хуки безопасности, которые реально срабатывают.** Хуки (PowerShell и bash, полный паритет функциональности) сканируют каждую Bash-команду, запись файла и промпт на секреты (Stripe, AWS, GitHub, Anthropic, Slack, npm и другие) и на опасные паттерны (`rm -rf`, `mkfs`, `git push --force`, `format C:`) — и **блокируют** до выполнения инструмента, а не после.
- **Doctor, а не просто документация.** Один скрипт (`doctor.ps1` / `doctor.sh`) проверяет структуру, гоняет eval-фикстуры, скармливает хукам заведомо плохой синтетический ввод, чтобы поймать тихие регрессии, и автоматически коммитит реестр в git — чтобы плохая правка всегда была откатываемой.
...
### Вариант A — как плагин Claude Code (быстрее всего, macOS/Linux)

Хуки безопасности, скиллы и слэш-команды двумя командами — без копирования файлов и ручной правки `settings.json`:
...
Плагин ставится **выключенным** (`defaultEnabled: false`), потому что его хуки могут блокировать вызовы инструментов — включайте осознанно через `/plugin`. Авто-подключённые хуки таргетят **bash** и требуют `jq` + `grep -P` в `PATH`. Windows: включайте хуки через PowerShell-обёртку ниже (одна `hooks.json` не ветвится по ОС). Свод правил и реестр на 2000+ скиллов ставятся per-project скриптами ниже.

### Вариант B — per-project скриптами (Windows-first, вся система)

Каждый скрипт существует в двух версиях с полным функциональным паритетом — выбирайте под свою ОС:

| Платформа | Требуется |
|---|---|
| **Windows** | PowerShell 5.1+ (скрипты `.ps1`, без доп. зависимостей) |
| **Linux / macOS** | `bash` (3.2+, так что дефолтный macOS `/bin/bash` работает), `jq` и `grep -P` (PCRE — стандартно на Linux; на macOS: `brew install grep` + `SREDNOFF_GREP_BIN=ggrep`, либо через WSL) |
